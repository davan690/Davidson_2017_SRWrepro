...
```{r missing intervals plot, echo=FALSE, fig.height=5.5, fig.width=4.5, message=FALSE, warning=FALSE}
#Basic plots 
par(mfrow=c(3,1))
...
```{r referee_comment_1_plot, echo=FALSE}
...
glimpse(pwr.analysis)

#plot data
ggplot(data = pwr.analysis, aes(y = Power, x = Mean.difference)) +
  geom_point() +
  ggtitle("Raw data result plot")
```

This is not exactly what the reviewer asked for but it is the same results as below (which is in the terms the reviewer asked for):

```{r referee_comment_2_plot, echo=FALSE}
#sort data into ggplot format
...
#filter(pwr.analysis.1, Alpha > 0.05)

#plot data
ggplot(data = pwr.analysis.1, aes(x = Mean.estimate, y = Alpha)) +
  geom_line(size = 1.5) +
  geom_vline(xintercept = 3.71, col = "blue") +
...
<span style="color:red">if the sampling continues over the next 8 years, the number of observations will not increase in a linear function as the number of individuals avaliable for recapture is growing (more individuals in database) at a different rate to the captue probability (that is hopefully constant). I will do the simulation recommmended because both the editor and reviewer wanted it.</span>
...
<span style="color:red">**"The existing simulation could be improved by assuming imperfect detection of calving events. For example, how does the variance around the estimate of mean calving interval change when 5% of calving events are not detected, increasing the number of 5 or 6 year observed calving intervals?"**</span>
...
I only have data for these overall data points up to 2012 so have done the analysis with this and can just add an additional year when I get it.
